...
class Chat:
	...
	def proses(self,data):
		...
		try:
			...
			if (command=='auth'):
				...
				usernameform = self.sessions[sessionid]['username']
				login.warning("SEND: session {} send message from {} to {}" . format(sessionid, usernamefrom, group_name))
	...
	def send_group_message(self, sessionid, username_from, group_name, message):
		if sessionid not in self.sessions:
			return {'status': 'ERRPR', 'message': 'Session Tidak Ditemukan'}
			...
			for username_dest in pemain_bola:
				if username_dest != username_from:
					...
					try:
						...
						outqueue_sender[username_from].put(notif)
					try:
						inqueue_receiver[usename_from].put(notif)
					except KeyError:
						inqueue_receiver[username_from]=Queue()
